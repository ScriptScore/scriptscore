...
| `torch` 2.13.0 | Python runtime | `Apache-2.0 AND Apache-2.0 WITH LLVM-exception AND BSD-2-Clause AND BSD-3-Clause AND BSL-1.0 AND MIT` | approved | Approve the current Torch wheel license expression for the AGPL client runtime. `BSL-1.0` is the OSI-approved permissive Boost Software License; the remaining terms are already accepted by the generator policy. Native libraries are reviewed through the native runtime provenance manifest. | Include generated notices and bundled native-library attributions from the wheel metadata. Reopen review if the Torch version or license metadata changes. | Codex / 2026-09-04 | Torch 2.13.0 wheel `METADATA` and bundled license files; SPDX `BSL-1.0`; `docs/licensing/native-runtime-provenance.json`. |
