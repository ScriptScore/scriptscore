...
| Paddle OCR det/rec model files under `cli/models/paddle/` | Resolved as `approved`. The bundled files match the official PP-OCRv5 mobile detection and recognition model repositories at pinned Hugging Face commits, with local checksums and Apache-2.0 redistribution terms recorded in the Paddle OCR model provenance manifest. | Verify generated `sbom-assets.json` and `THIRD_PARTY_NOTICES.md` include `paddle-ocr-model-asset` entries when legal artifacts are regenerated. Re-open review if model files are replaced or upstream license evidence changes. |
