...
| `aistudio-sdk` | Python runtime | Unknown upstream wheel metadata | pending | Upstream metadata does not provide a usable license expression. Approve only after upstream license evidence is recorded or remove dependency. | Record upstream license evidence or replace before release. | pending reviewer / pending date | Upstream package metadata; generated `license-policy-report.json`. |
...
| --- | --- | --- |
| `aistudio-sdk` | Treat as `replace_before_release` unless upstream license evidence is obtained. PyPI currently provides no usable license metadata or source distribution for the reported wheel. | Remove the dependency from the distributed runtime, replace it, or record upstream license permission from an authoritative source. |
...
| `eslint-plugin-sonarjs` | Replace or remove before release-quality source publication. Package metadata says `LGPL-3.0-only`, but the installed package also contains a Sonar Source-Available license with field-of-use restrictions. | Prefer removing this dev dependency or pinning/replacing with a package whose metadata and included license files agree. Do not bundle it. |
