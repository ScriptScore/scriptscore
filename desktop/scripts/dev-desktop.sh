...
if [[ -n "${DESKTOP_FRONTEND_URL:-}" ]]; then
  TAURI_DEV_CONFIG="{\"build\":{\"devUrl\":\"${DESKTOP_FRONTEND_URL}\"},\"app\":{\"security\":{\"devCsp\":{\"img-src\":[\"'self'\",\"data:\",\"asset:\",\"${DESKTOP_FRONTEND_URL}\",\"http://asset.localhost\",\"https://asset.localhost\"]}}}}"
